...
JumpChargeBar::JumpChargeBar(Object* Owner) :UI(UT_JUMPCHABAR)
{
    m_pOwner = Owner;
}
...
{
	if (dynamic_cast<Player*>(m_pOwner)->IsSpacePressed())
	{
		...
		Rectangle(hDC, m_tRect.left,
			m_tRect.top,
			m_tRect.left + static_cast<INT>(m_tPos.fCX * dynamic_cast<Player*>(m_pOwner)->Get_JumpCharge()),
			m_tRect.bottom);
